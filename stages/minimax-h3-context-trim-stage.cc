...
std::uint8_t*
MiniMaxH3ContextTrimStage::frame_pixels_(BeatPayloadIntf* beat,
                                         std::int64_t* samples)
{
  auto* tb = dynamic_cast<TensorBeatPayload*>(beat);
  if (tb == nullptr || tb->dtype != TensorBeat::DType::U8 ||
      tb->shape.empty() || !tb->is_contiguous()) {
    return nullptr;
  }
  std::int64_t n = 1;
  for (std::int64_t d : tb->shape) {
    if (d <= 0 || n > (std::int64_t)(1 << 30) / d) { return nullptr; }
    n *= d;
  }
  *samples = n;
  return tb->as_u8();
...
MiniMaxH3ContextTrimStage::luma_match_()
{
  ...
  for (int k = 0; k < n; ++k) {
    px[(std::size_t)k] =
        frame_pixels_(_held[(std::size_t)k].get(), &len[(std::size_t)k]);
