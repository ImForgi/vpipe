...
// The pixels of a planar u8 frame beat, with its shape, or nullptr when
// the beat is not one. Every stage that walks decoded frames needs
// exactly this, and they need the SAME bound on it: the element count
// has to fit the `int` arithmetic done on h and w.
//
// It checks the LAYOUT, not the size. A measure that needs a minimum
// frame to mean anything says so itself -- the levelling measures below
// return 0 under 3x3 -- because a caller that only walks the samples,
// like the luma match, is right to take any frame it is given.
std::uint8_t* frame_pixels(BeatPayloadIntf* beat, int* channels, int* h,
                           int* w, std::int64_t* samples);
